...
const App: React.FC = () => {
  ...
  const handlePaperCreated = async (newPaper: QuestionPaper): Promise<'success' | 'saved_without_file' | 'failed'> => {
      // Edit handler for updating a paper
      const handlePaperEdited = async (updatedPaper: QuestionPaper) => {
        setPapers(prev => {
        ...
        if (!isOffline) {
          try {
            ...
            console.warn('Cloud update failed, only local copy updated.', e);
          }
        }
      };
    if (isOffline) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {role === 'teacher' ? <TeacherView onPaperCreated={handlePaperCreated} onPaperDeleted={handlePaperDeleted} onPaperEdited={handlePaperEdited} existingPapers={papers} submissions={submissions} /> : <StudentView papers={papers} activeStudent={activeStudent} onLogin={handleStudentLogin} registeredStudents={students} onSubmission={handleSubmissionReceived} />}
    </div>
  );
};
